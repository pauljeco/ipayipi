...
#'  maximum value.
#' @param robust Use the median instead of the mean to calculate the
#'  central tendancy of the window of interest. Reduces the influence
#'  of outliers on drift parametization.
...
gw_driffter <- function(
  ...
  slist <- ipayipi::dta_list(input_dir = input_dir, recurr = recurr,
    baros = FALSE, wanted = wanted, unwanted = unwanted, prompt = prompt,
    file_ext = ".rds"
  )
  if (length(slist) == 0) {
    stop("There are no such standardises water level files in the directory!")
  }
  cr_msg <- padr(
    core_message = paste0(" linear drift correction ", collapse = ""),
  ...
  sapply(slist, FUN = function(emm) {
    ...
    if (nrow(dtum) > 0 & nrow(rlog) > 0 & nrow(sol_temp$log_t) > 0) {
      ...
      drifting_ttt <- merge(y = rlog_reads_t, x = drifting_tt, all.x = TRUE,
        by = "Date_time"
      )
      drifting_t <- drifting_ttt[,
        c("Date_time", "ut1_level_m", "level_kpa", "bt_level_m", "bt_Outlier",
          "bt_Outlier_rule", "t_bt_level_m", "Drift_offset_m", "t_Outlier",
          "t_Outlier_rule", "t_level_m", "H_masl", "dips"
        ), with = TRUE
      ]
      ...
      callbs <- future.apply::future_lapply(seq_len(dippers),
        function(z, d_tab = drifting_t, rng_side_in = rng_side,
          ...
          if (c_row >= 1) {
            win_row_v <- win_rows(
              dta = d_tab$t_bt_level_m, rng_side = rng_side_in,
      ...
      drifting_t[c(callbs$row_ns), "dip_n"] <- ifelse(
        is.na(drifting_t[c(callbs$row_ns), "dip_offset"]),
        NA, drifting_t[c(callbs$row_ns), ]$dip_n
      )
      # Set possi to NA if there is no dip_offset
      drifting_t[c(callbs$row_ns), "possi"] <- ifelse(
        is.na(drifting_t[c(callbs$row_ns), "dip_offset"]),
      ...
      if (possi_cat[1] != -1) {
        for (dof in seq_along(possi_cat)) {
          ...
          t_diff <- as.numeric(drifting_t$Date_time[ac_e]) -
  ...
  cr_msg <- padr(
    core_message = paste0(" drift corrected ", collapse = ""),
    pad_char = "=", pad_extras = c("|", "", "", "|"),
    force_extras = TRUE, justf = c(0, 0), wdth = 80
  )
  message(cr_msg)
